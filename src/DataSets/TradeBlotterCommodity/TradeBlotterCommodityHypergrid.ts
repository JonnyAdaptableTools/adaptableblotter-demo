...
export var TradeBlotterCommodity: IDataSetConfiguration = {
    ...
    setGridProperties: (grid: any) => {
        ...
        behavior.dataModel.getCellEditorAt = function (columnIndex: any, rowIndex: any, declaredEditorName: any, options: any) {
            ...
            if (options.column.name !== TradeBlotterCommodity.primaryKey) {
                editorName = 'textfield';
            }
            return grid.cellEditors.create(editorName, options);
        }
        HelperHypergrid.FormatColumns([1, 2, 6], behavior, "shortDateFormat")
        HelperHypergrid.FormatColumns([3], behavior, "USDCurrencyFormat")
    },
    tickData: (grid: any) => {
